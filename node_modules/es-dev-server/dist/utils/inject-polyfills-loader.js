...
function findScripts(cfg, documentAst) {
    const scriptNodes = building_utils_1.findJsScripts(documentAst, cfg.polyfillsLoaderConfig && cfg.polyfillsLoaderConfig.exclude);
    const files = [];
    const inlineScripts = [];
...
async function transformInlineScripts(cfg, inlineScriptNodes) {
    ...
    for (const scriptNode of inlineScriptNodes) {
        ...
        const filePath = cfg.indexFilePath.endsWith(path_1.default.sep)
            ? path_1.default.join(cfg.indexFilePath, 'index.html')
            : cfg.indexFilePath;
        const asyncTransform = cfg
            .transformJs({
            filePath,
            uaCompat: cfg.uaCompat,
            code: dom5_fork_1.getTextContent(scriptNode),
            transformModule: false,
        })
            .then((code) => {
            dom5_fork_1.setTextContent(scriptNode, code);
        });
...
async function injectPolyfillsLoader(cfg) {
    ...
    if (!hasPolyfills(polyfillsLoaderConfig.polyfills) && !polyfillModules) {
        ...
        if (inlineScripts && inlineScripts.length > 0) {
            ...
            return { indexHTML: parse5_1.serialize(documentAst), inlineScripts, polyfills: [] };
        }

...
function createConfig(config) {
    const { babel = false, babelConfig, babelExclude = [], babelModernExclude = [], babelModuleExclude = [], basePath, compress = true, fileExtensions: fileExtensionsArg, hostname, http2 = false, logStartup, open = false, port, sslCert, sslKey, watch = false, logErrorsToBrowser = false, polyfillsLoader, responseTransformers = [], debug = false, nodeResolve: nodeResolveArg = false, dedupeModules, moduleDirs = ['node_modules', 'web_modules'], preserveSymlinks = false, onServerStart, } = config;
    if (debug) {
    ...
    const fileExtensions = [...(fileExtensionsArg || []), ...building_utils_1.defaultFileExtensions];
